...
// ====== ROW: sub-kolom per-produk (hormati izin) ======
function PersonRow({
  ...
  return (
    <tr className="border-t align-top">
      ...
      {productConfigs.map(cfg => {
        ...
        if (!isAllowed) return (
          <td key={cfg.name} className="p-2 align-top text-slate-400">—</td>
        );
...
function Overview({
  ...
  const ProductHeads = ({ list }: { list: ProductConfig[] }) => (
    <>
      {list.map(cfg => (
        <th key={cfg.name} className="p-2 w-[22%] text-right">{cfg.name}</th>
      ))}
    </>
  );
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      <Section title="Ringkasan MBM (SGP)">
        ...
        {colsMBM.length === 0 && (
          <div className="text-xs text-slate-500 mt-2">Tidak ada produk yang diizinkan di unit ini.</div>
        )}
      </Section>
      ...
      {!pinOk ? (
        ...
        <>
          <Section
            ...
            extra={<button className="px-3 py-1.5 rounded-lg border" onClick={() => setPinOk(false)}><Lock size={14} /> Kunci</button>}
          >
            <div className="grid md:grid-cols-4 gap-3 items-end">
              ...
              {/* Produk: dropdown dari "Kelola Kolom Produk" + difilter izin */}
              <div>
              ...
              <div className="md:col-span-4">
                <button
                  className={`px-3 py-2 rounded-xl ${canAdd ? "bg-indigo-600 text-white" : "bg-slate-200 text-slate-500"} flex items-center gap-2`}
                  onClick={() => { if (canAdd) addAchievement(); }}
                  disabled={!canAdd}
                  title={canAdd ? "" : "Lengkapi data dan pastikan produk diizinkan untuk pegawai tersebut"}
                >
                  <Plus size={16} /> Tambah
                </button>
              </div>
          ...
          <Section title="Kelola Kolom Produk (Target & Progress)">
            <div className="grid md:grid-cols-4 gap-3 items-end">
              ...
              <div>
                <button className="px-3 py-2 text-white rounded-xl bg-white border" onClick={addProduct}>
                  Tambah Kolom
                </button>
              </div>
            </div>
              ...
              <div className="mt-3 grid md:grid-cols-3 gap-2">
                {productConfigs.map(cfg => (
                  <div key={cfg.name} className="px-3 py-2 rounded-xl border flex items-center justify-between">
                    ...
                    <button className="p-1 text-white rounded-lg border" onClick={() => removeProduct(cfg.name)} title="Hapus kolom">
                      <X size={14} />
                    </button>
                  </div>
                ))}
            ...
            <table className="w-full table-fixed text-sm">
              <thead className="bg-slate-50 text-left">
                <tr>
                  ...
                  {productConfigs.map(cfg => (
                    <th key={cfg.name} className="p-2 text-center">{cfg.name}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
            ...
            <table className="w-full table-fixed text-sm">
              <thead className="bg-slate-50 text-left">
                <tr>
                  ...
                  <th className="p-2 w-[18%]">Role</th>
                  {productConfigs.map(cfg => (
                    <th key={cfg.name} className="p-2 text-right">{cfg.name}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
          ...
          <Section title="Kelola Pegawai">
            <div className="grid md:grid-cols-5 gap-3 items-end">
              ...
              <div>
                <button className="px-3 py-2 text-white rounded-xl bg-white border" onClick={addEmployee}>
                  Tambah Pegawai
                </button>
              </div>
            </div>
            ...
            <div className="mt-4">
              ...
              <div className="text-xs text-white text-slate-500 mt-2">
                ...
                {ach.slice(-50).reverse().map(a => {
                  ...
                  return (
                    <tr key={a.id} className="border-t align-top">
                      ...
                      <td className="p-2 text-right whitespace-nowrap">{nfmt(a.amount)}</td>
                      <td className="p-2">
                        <button className="px-2 py-1 text-white rounded-lg border" onClick={() => removeAchievement(a.id)}>Hapus</button>
                      </td>
                    </tr>
                  );
                })}
...
export default function App() {
  ...
  // sinkronisasi saat daftar produk berubah (buat key utk semua pegawai)
  useEffect(() => {
    ...
    <div className="min-h-screen w-full bg-slate-50 text-slate-900">
        ...
        <div className="w-full px-6 py-3 flex items-center gap-4">
          ...
          <div className="flex items-center gap-3 shrink-0">
            ...
            <div>
              ...
              <div className="text-xs text-slate-500">Localhost • BM input only (PIN)</div>
            </div>
          </div>
      ...
      <main className="w-full px-6 py-4 space-y-4">
        ...
        <footer className="pt-6 text-sm text-slate-500 flex items-center gap-2">
          <CheckCircle2 size={16} /> Data & konfigurasi tersimpan lokal (localStorage). Siap diupgrade ke DB/REST kapan saja.
        </footer>
      </main>
    </div>
  );
